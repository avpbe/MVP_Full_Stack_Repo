...
const updateDashboard = () => {
    ...
    renderEngineersList(); // Renderiza a lista de engenheiros
};
...
const renderEngineersList = () => {
    const list = document.getElementById('engineers-list');
    list.innerHTML = '';
    if (localEngineers.length === 0) {
        list.innerHTML = '<p class="text-muted p-3">Nenhum engenheiro cadastrado.</p>';
        return;
    }
...
const addRequest = (projectName, discipline, description, engineer, startDate, endDate) => {
    if (new Date(startDate) >= new Date(endDate)) return alert("Erro: A data de fim deve ser posterior à data de início.");
    if (localRequests.some(req => req.nome_projeto.toLowerCase() === projectName.toLowerCase())) return alert(`Erro: A requisição "${projectName}" já existe.`);
    
    const engineerProjects = localRequests.filter(p => p.engineer === engineer);
    const newStart = new Date(startDate);
    const newEnd = new Date(endDate);
    const hasConflict = engineerProjects.some(p => (newStart < new Date(p.endDate) && newEnd > new Date(p.startDate)));

    if (hasConflict) return alert(`Erro: O engenheiro ${engineer} já está alocado em um projeto nesse período.`);
...
const addEngineer = (name, role, discipline, platformRole) => {
    if (localEngineers.some(eng => eng.name.toLowerCase() === name.toLowerCase())) return alert(`Erro: O engenheiro "${name}" já está cadastrado.`);
...
const deleteEngineer = (engineerName) => {
    if (confirm(`Tem certeza que deseja remover o engenheiro "${engineerName}"? Esta ação não pode ser desfeita.`)) {
        // Opcional: Desatribuir projetos do engenheiro deletado
        localRequests.forEach(req => {
            if (req.engineer === engineerName) {
                req.engineer = null; // ou 'Não atribuído'
            }
        });
        setLocalData('localRequests', localRequests);
...
const initializeApp = () => {
    ...
    // Configura o modal de engenheiros
    const engineerModalEl = document.getElementById('newEngineerModal');
